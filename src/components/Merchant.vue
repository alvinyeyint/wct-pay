<template>
  <section class="container max-w-7xl mx-auto py-20 bg-white">
    <div class="grid grid-cols-6 gap-6 px-4 sm:px-6 lg:px-8">
      <div class="col-span-6 md:col-span-3 flex items-center justify-center">
        <img src="../assets/merchant.svg" class="object-cover w-4/5" />
      </div>
      <div class="col-span-6 md:col-span-3 flex flex-col justify-center">
        <div>
          <h3 class="text-subtitle">
            Enabling Merchants with <br />
            <span class="text-primary-600"
              >Seamless Crypto Payment <br />
              Solutions</span
            >
          </h3>
          <div class="flex gap-4 items-start mt-8">
            <img
              src="../assets/icon/check-icon.svg"
              alt="check-icon"
              class="size-8"
            />
            <div>
              <p class="text-gray-500 font-bold">Accepting Crypto Payments</p>
              <p class="text-gray-500 mt-2">
                WCT Pay’s crypto payment solutions enable merchants to easily
                accept cryptocurrency payments from customers around the world.
                Our platform supports a wide range of digital currencies, giving
                merchants the flexibility to cater to diverse customer
                preferences.
              </p>
            </div>
          </div>
          <div class="flex gap-4 items-start mt-6">
            <img
              src="../assets/icon/check-icon.svg"
              alt="check-icon"
              class="size-8"
            />
            <div>
              <p class="text-gray-500 font-bold">
                Customisable Settlement Options
              </p>
              <p class="text-gray-500 mt-2">
                We offer global multi-currency settlement to any bank account,
                ensuring merchants access their funds quickly and conveniently.
                With WCT Pay, merchants can choose the settlement option that
                suits their needs, empowering them to manage their payments
                efficiently.
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>
</template>
